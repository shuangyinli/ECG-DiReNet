...
class Model(nn.Module):
    def __init__(self, device, number_of_diffusions=1000, kernel_size=5, num_levels=3, n_channels=1, resolution=2048,load_path=None):
        super().__init__()
        self.device = device
        self.UnetModel = ECGunetChannels(
        ...
        self.ECGNetmodel = ECGNet(n_classes=1, channels=1, samples=2048, dropout=0,
    ...
    def load_weights(self,load_path):
        self.UnetModel.load_state_dict(torch.load(load_path, map_location=self.device))

    def freeze_downsampling(self):
        for param in self.UnetModel.downsampling_blocks.parameters():
            param.requires_grad = False

    def adjust_output_conv(self):
        self.UnetModel.output_conv = nn.Sequential(
        ...
        self.UnetModel = self.UnetModel.to(self.device)

    def forward(self, x):
        unet_input = x.permute(0, 2, 1)
        unet_out = self.UnetModel(unet_input)
