...
from utils.utils import EMA, MSE, wavelet_transform, Normalization2, smooth_continuous_outliers
...
class DiffusionModel(nn.Module):
    def __init__(self, time_steps=1000, n_channels=1, device="cpu"):
        ...
        self.betta = torch.linspace(1e-4, 0.02, time_steps, device=device)
    ...
    def forward_diffuse(self, x0, t, noise=None):
        ...
        alpha_bar_sqrt_t = self.alpha_cumprod_sqrt[t].to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--checkpoint_path', type=str,
                        default="../weight/classification_model.pth",
    ...
    transformed_data = np.array([wavelet_transform(x) for x in tqdm(normalized_data, desc="Applying wavelet transform")])
    
    labels = np.zeros(transformed_data.shape[0]) + args.label

    print(f"Processed data shape: {transformed_data.shape}")

    np.savez(args.save_path, data=transformed_data, label=labels)
